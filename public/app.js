...
(function () {
  ...
  function loadForRole() {
    if (S.auth.role === "admin" || S.auth.role === "superadmin") {
      return Promise.all([api("GET", "/api/facilities"), api("GET", "/api/users"), api("GET", "/api/reports"), api("GET", "/api/leads"), api("GET", "/api/payables"), api("GET", "/api/recurring")])
        .then(function (r) { S.facilities = r[0]; S.users = r[1]; S.reports = r[2]; S.leads = r[3]; S.payables = r[4]; S.recurring = r[5]; });
    }
  ...
  function facInfo(f) {
    ...
    return '<div class="between" style="margin-top:12px"><h4 style="margin:0">Edit information</h4><button class="btn btn-ok sm" data-a="facdone">' + I("check") + ' Done</button></div><div class="stack" style="margin-top:8px">'
      ...
      + notesCard + '<button class="btn btn-ok" data-a="facdone" style="width:100%;justify-content:center">' + I("check") + " Done editing</button></div>";
  }
  ...
  function facAudit(f) {
    ...
    return '<div class="stack" style="margin-top:12px">'
      + '<div class="card" style="border-color:' + (due ? "var(--hazard-d)" : "var(--line)") + '"><div style="font-weight:700">' + I("clipboard-check") + " Monthly audit" + (due ? ' \u2014 <span style="color:var(--hazard-d)">due now</span>' : "") + '</div><p class="hint" style="margin:6px 0 0">A more in-depth monthly check: every vacant unit gets full checks and a photo, every occupied unit is confirmed to have a customer lock, and 10 facility photos are required. ' + esc(when) + ".</p>"
      ...
      + grp("Master unit list", "list-numbers", '<p class="hint" style="margin:0 0 8px">List every unit at this facility. On an audit, anything here not in the Vacant list (Site check tab) is treated as occupied and the tech confirms a customer lock is on it.</p>' + listEditor(gA(f, "units"), "units", [{ k: "unit", ph: "Unit #", unit: 1 }], "Add unit"))
      + "</div>";
  }
  ...
  function moveInTracker() {
    ...
    rented.forEach(function (l) {
      ...
      byMonth[key].total++; byMonth[key].emp[emp] = (byMonth[key].emp[emp] || 0) + 1;
    });
    var months = Object.keys(byMonth).sort().reverse();
    return '<p class="hint" style="margin-bottom:8px">Move-ins by month, credited to whoever logged the move-in.</p>' + months.map(function (mk) {
      var m = byMonth[mk], emps = Object.keys(m.emp).sort(function (a, b) { return m.emp[b] - m.emp[a]; });
      return '<div class="card"><div class="between"><div style="font-weight:700">' + esc(m.label) + '</div><span class="pill" style="background:#E1F0E9;color:var(--ok)">' + m.total + " moved in</span></div><div class=\"dgrp\" style=\"margin-top:8px\">" + emps.map(function (e) { return '<div class="dline" style="display:flex;justify-content:space-between;align-items:center"><span>' + esc(e) + '</span><span style="font-weight:700">' + m.emp[e] + "</span></div>"; }).join("") + "</div></div>";
    }).join("");
  }
  function leadsView() {
    ...
    var bt = [["warm", "Warm"], ["cold", "Cold"], ["movedin", "Moved in"], ["todo", "To-do"], ["tracker", "Move-in tracker"]];
  ...
  function payCard(p, actions) {
    return '<div class="card"><div class="row" style="justify-content:space-between;align-items:flex-start"><div style="font-weight:700">' + esc(facName(p.facilityId)) + '</div><span class="muted" style="font-size:12px">Logged ' + fdt(p.createdAt) + '</span></div>'
      ...
      + (p.invoice ? '<a href="' + esc(p.invoice) + '" target="_blank" rel="noopener" class="muted" style="font-size:12px;display:inline-block;margin-top:6px">' + I("paperclip") + " View attached invoice</a>" : "")
      + (actions || "") + "</div>";
  }
  function payableForm() {
    ...
    return '<div class="stack">'
      + '<div class="card"><div class="label">Facility</div><select data-pay="facilityId"><option value="">— pick a facility —</option>' + facOpts + "</select>"
      ...
      + '<div class="label">Mailing address</div><textarea rows="2" data-pay="mailingAddress">' + esc(d.mailingAddress || "") + "</textarea>"
      + '<div class="label">Attach invoice (optional)</div>' + (d.invoice ? '<div class="row" style="gap:8px;align-items:center"><img src="' + esc(d.invoice) + '" style="height:54px;border-radius:8px;border:1px solid var(--line)"><button class="btn sm danger" data-a="payinvoicedel">' + I("trash") + ' Remove</button></div>' : '<label class="addph" style="display:inline-flex">' + I("paperclip") + 'Attach photo<input type="file" accept="image/*" style="display:none" data-payinvoice></label>')
  ...
  function payExpenses() {
    var appr = (S.payables || []).filter(function (p) { return p.status === "approved"; });
    if (!appr.length) return '<div class="empty">No approved expenses yet.</div>';
    var byFac = {};
    ...
    return Object.keys(byFac).map(function (fid) {
      return '<div class="dh" style="margin-top:8px">' + esc(facName(fid)) + "</div><div class=\"stack\" style=\"margin-top:6px\">" + byFac[fid].map(function (p) {
        return payCard(p, '<button class="check full' + (p.billedBack ? " on" : "") + '" data-a="paybilled" data-id="' + p.id + '" style="margin-top:10px"><span class="bx">' + (p.billedBack ? I("check") : "") + "</span>Billed back to facility</button>");
      }).join("") + "</div>";
    }).join("");
  }
  ...
  function payablesView() {
    ...
    var st = [["pending", "Pending (" + pend + ")"], ["new", "New payable"], ["expenses", "Expenses"], ["recurring", "Recurring"]];
    var nav = '<div class="tabs">' + st.map(function (t) { return '<button class="' + (sub === t[0] ? "on" : "") + '" data-a="paysub" data-k="' + t[0] + '">' + t[1] + "</button>"; }).join("") + "</div>";
    var body = sub === "new" ? payableForm() : sub === "expenses" ? payExpenses() : sub === "recurring" ? recurringView() : payPending();
  ...
  document.addEventListener("change", function (ev) {
    ...
    if (t.getAttribute("data-payinvoice") !== null) { var files = Array.prototype.slice.call(t.files); t.value = ""; if (files[0]) compress(files[0]).then(function (im) { if (im) { S.payDraft = S.payDraft || {}; S.payDraft.invoice = im.url; render(); } }); return; }
  ...
  document.addEventListener("click", function (ev) {
    ...
    if (a === "paysave") { var pd = S.payDraft || {}; if (!pd.facilityId) { toast("Pick a facility."); return; } if (!pd.who) { toast("Choose on-site or sub contractor."); return; } api("POST", "/api/payables", pd).then(function () { S.payDraft = {}; S.paySub = "pending"; return reloadPayables(); }).then(function () { toast("Payable submitted."); render(); }).catch(function (e) { toast(e.message); }); return; }
    ...
    if (a === "recadd") { var rd = S.recDraft || {}; if (!rd.facilityId) { toast("Pick a facility."); return; } api("POST", "/api/recurring", rd).then(function () { S.recDraft = {}; return reloadRecurring(); }).then(function () { toast("Recurring payment added."); render(); }).catch(function (e) { toast(e.message); }); return; }
    if (a === "recdel") { if (!confirm("Delete this recurring payment?")) return; api("DELETE", "/api/recurring/" + id).then(function () { return reloadRecurring(); }).then(render).catch(function (e) { toast(e.message); }); return; }
