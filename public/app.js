...
(function () {
  ...
  var S = {
    ...
    invoices: [], invDraft: {},
    deposits: [], depDraft: { entries: [] }, depSub: "new", depStage: "entry",
  };
  ...
  function loadForRole() {
    if (S.auth.role === "admin" || S.auth.role === "superadmin") {
      var calls = [api("GET", "/api/facilities"), api("GET", "/api/users"), api("GET", "/api/reports"), api("GET", "/api/leads"), api("GET", "/api/deposits")];
      if (S.auth.role === "superadmin") calls.push(api("GET", "/api/payables"), api("GET", "/api/recurring"), api("GET", "/api/expenses"), api("GET", "/api/billback"), api("GET", "/api/invoices"));
      return Promise.all(calls).then(function (r) { S.facilities = r[0]; S.users = r[1]; S.reports = r[2]; S.leads = r[3]; S.deposits = r[4]; if (r[5]) S.payables = r[5]; if (r[6]) S.recurring = r[6]; if (r[7]) S.expenses = r[7]; if (r[8]) S.billback = r[8]; if (r[9]) S.invoices = r[9]; });
    }
  ...
  function moveInTracker() {
    ...
    if (!rented.length) return '<div class="empty">No move-ins recorded yet.</div>';
    var byMonth = {};
    ...
    return '<p class="hint" style="margin-bottom:8px">Move-ins by month, credited to whoever logged the move-in. Tap an employee to see who they moved in.</p>' + months.map(function (mk) {
  ...
  function leadsView() {
    ...
    return sfCard + '<div class="between"><h3>Lead tracking</h3><button class="btn btn-dark sm" data-a="addlead">' + I("plus") + ' Add new lead</button></div><p class="hint">Track potential customers and their follow-ups.</p>' + tabs + content;
  }
  function employee() {
  ...
  function addExpenseForm() {
    var d = S.expDraft || {};
    var facOpts = '<option value=""' + (!d.facilityId ? " selected" : "") + ">General (no facility)</option>" + S.facilities.map(function (f) { return '<option value="' + f.id + '"' + (d.facilityId === f.id ? " selected" : "") + ">" + esc(f.name) + "</option>"; }).join("");
    return '<button class="btn" data-a="expback" style="margin-bottom:10px">' + I("arrow-left") + ' Expenses</button><div class="card">'
      + '<div class="label">Facility</div><select data-exp="facilityId">' + facOpts + '</select><p class="hint" style="margin:4px 0 0">Leave as \u201cGeneral\u201d for an expense not tied to a facility.</p>'
      + '<div class="label">Amount</div><input data-exp="amount" value="' + esc(d.amount || "") + '" placeholder="$">'
      + '<div class="label">Source of expense</div><div class="cond"><button class="' + (d.source === "American Express" || !d.source ? "ok" : "") + '" data-a="expsrc" data-v="American Express">American Express</button><button class="' + (d.source === "Other" ? "ok" : "") + '" data-a="expsrc" data-v="Other">Other</button></div>'
      + (d.source === "Other" ? '<div class="label">Source (type it)</div><input data-exp="sourceOther" value="' + esc(d.sourceOther || "") + '">' : "")
      + '<div class="label">Description</div><textarea rows="2" data-exp="description">' + esc(d.description || "") + "</textarea>"
      + '<div class="label">Date payment was made</div><input type="date" data-exp="datePaid" value="' + esc(d.datePaid || "") + '">'
      + '<button class="btn btn-dark" data-a="expsave" style="width:100%;justify-content:center;margin-top:12px">' + I("plus") + " Save expense</button></div>";
  }
  ...
  function billbackConfig() {
    return '<p class="hint">Configure each facility\u2019s monthly bill-back. Management and software fees are per unit, multiplied by the master unit count.</p>'
      + S.facilities.map(function (f) {
        var units = (f.config.units || []).length;
        var bb = S.billback[f.id] || { mgmtRate: "", softwareRate: "", onsiteFee: "", otherFees: [] };
        var mgmt = parseFloat(bb.mgmtRate || 0) * units, soft = parseFloat(bb.softwareRate || 0) * units;
        var others = bb.otherFees || [];
        return '<div class="card"><div class="between"><div style="font-weight:700">' + esc(f.name) + '</div><span class="pill">' + units + " units</span></div>"
  ...
  function feeListFor(fid) {
    ...
    var fac = S.facilities.find(function (f) { return f.id === fid; }); var units = fac ? (fac.config.units || []).length : 0;
  ...
  function configureBillBack() {
    ...
    return head
      + '<div class="card"><div class="dh">One-time expenses to bill</div>' + (ckPay || ckExp || '<div class="muted" style="font-size:13px">None awaiting bill-back.</div>') + (ckPay + ckExp) + "</div>"
  ...
  function invoicesView() {
    ...
    return '<p class="hint" style="margin:0 0 8px">Generated invoices awaiting bill-back. Mark one billed back to move it to Paid invoices.</p>' + (open.length ? '<div class="stack">' + open.map(function (i) { return invCard(i, false); }).join("") + "</div>" : '<div class="empty">No open invoices. Create one from Expenses \u2192 Configure bill back.</div>');
  }
  ...
  function buildInvoicePDF(inv) {
    ...
    doc.text("Copper River LLC", 56, y + 26); doc.text("P.O. Box 568", 56, y + 40); doc.text("Augusta, KS 67010", 56, y + 54);
    ...
    var ty = by + 78;
    doc.setFont("helvetica", "bold"); doc.text("Description", 56, ty); doc.text("Amount", 470, ty); doc.line(56, ty + 6, 540, ty + 6);
    doc.setFont("helvetica", "normal"); var ly = ty + 24;
    (inv.lineItems || []).forEach(function (li) {
      var lines = doc.splitTextToSize(String(li.description || ""), 390);
      doc.text(lines, 56, ly); doc.text(money(li.amount), 470, ly);
  ...
  function depNew() {
    ...
    var rows = d.entries.map(function (e) {
      return '<div class="card"><div class="between"><div class="label" style="margin:0">Payment</div><button class="icon-btn danger" data-a="depdel" data-id="' + e.id + '">' + I("trash") + "</button></div>"
        + '<div class="label">Facility</div><select data-de="' + e.id + '|facilityId">' + facOpts(e.facilityId) + "</select>"
        + '<div class="label">What it\u2019s for</div><input data-de="' + e.id + '|forWhat" value="' + esc(e.forWhat || "") + '">'
        + '<div class="label">Customer</div><input data-de="' + e.id + '|customer" value="' + esc(e.customer || "") + '">'
        + '<div class="two-col"><div><div class="label">Type</div><div class="cond"><button class="' + (e.type !== "MO" ? "ok" : "") + '" data-a="detype" data-id="' + e.id + '" data-v="CK">CK</button><button class="' + (e.type === "MO" ? "ok" : "") + '" data-a="detype" data-id="' + e.id + '" data-v="MO">MO</button></div></div><div><div class="label">Amount</div><input data-de="' + e.id + '|amount" value="' + esc(e.amount || "") + '" placeholder="$"></div></div>'
        + "</div>";
    }).join("");
  ...
  function depCompleted() {
    ...
    var card = function (dep) {
      ...
      return '<div class="card"><div class="between"><div style="font-weight:700">Deposit \u00b7 ' + fdt(dep.createdAt) + '</div><span class="muted" style="font-size:12px">' + esc(dep.createdBy || "") + "</span></div>"
        ...
        + '<div class="kv" style="margin-top:6px"><span>Checks (CK)</span><span>' + money(t.byType.CK) + "</span><span>Money orders (MO)</span><span>" + money(t.byType.MO) + '</span><span>Total</span><span style="font-weight:700">' + money(t.grand) + "</span></div>"
        + '<div class="row" style="gap:8px;margin-top:10px">' + (dep.settled ? '<span class="pill" style="background:#E1F0E9;color:var(--ok)">' + I("check") + " Cash settled" + (dep.settledAt ? " " + fdt(dep.settledAt) : "") + "</span>" : '<button class="btn btn-ok sm" data-a="depsettle" data-id="' + dep.id + '">' + I("check") + " Cash is settled</button>") + '<button class="icon-btn danger" data-a="depdelete" data-id="' + dep.id + '">' + I("trash") + "</button></div></div>";
    };
  ...
  function payablesView() {
    ...
    var st = [["pending", "Pending (" + pend + ")"], ["new", "New payable"], ["expenses", "Expenses"], ["general", "General"], ["billedback", "Billed back"], ["billback", "Bill-back config"], ["recurring", "Recurring"], ["invoices", "Invoices"], ["paidinvoices", "Paid invoices"]];
    var nav = '<div class="tabs">' + st.map(function (t) { return '<button class="' + (sub === t[0] ? "on" : "") + '" data-a="paysub" data-k="' + t[0] + '">' + t[1] + "</button>"; }).join("") + "</div>";
    var body = sub === "new" ? payableForm() : sub === "expenses" ? payExpenses() : sub === "general" ? generalExpenses() : sub === "billedback" ? payBilledBack() : sub === "billback" ? billbackConfig() : sub === "recurring" ? recurringView() : sub === "configure" ? configureBillBack() : sub === "invoices" ? invoicesView() : sub === "paidinvoices" ? paidInvoices() : payPending();
    return '<h3>Payables</h3><p class="hint">Track contractor invoices from submission through approval, billing back, and recurring payments.</p>' + nav + body;
  }
  ...
  function reportsHome() {
    ...
    var now = Date.now(), MONTH = 30 * 86400000;
    var dueAudits = S.facilities.filter(function (f) { var anchor = f.config.lastAuditAt || f.config.setupAt || 0; return anchor && now >= anchor + MONTH; });
    var auditBanner = dueAudits.length ? ('<div class="card" style="border-color:var(--hazard-d);background:#FBEAE1;margin-bottom:14px"><div style="font-weight:700;color:var(--hazard-d)">' + I("alert-triangle") + ' Monthly audit due (' + dueAudits.length + ')</div><p class="hint" style="margin:6px 0 10px">Configure and send the audit from the facility\u2019s page, or disregard if it\u2019s already been done.</p>' + dueAudits.map(function (f) { return '<div class="row" style="justify-content:space-between;align-items:center;gap:8px;padding:6px 0;border-top:1px solid var(--line)"><span style="font-weight:600">' + esc(f.name) + '</span><span class="row" style="gap:6px"><button class="btn btn-dark sm" data-a="facconfig" data-id="' + f.id + '">' + I("settings") + ' Configure</button><button class="btn sm" data-a="auditdone" data-id="' + f.id + '">Disregard</button></span></div>'; }).join("") + '</div>') : "";
    return auditBanner + '<h3>Reports</h3>'
  ...
  document.addEventListener("click", function (ev) {
    ...
    if (a === "payapprove") { api("PUT", "/api/payables/" + id, { status: "approved" }).then(function () { return reloadPayables(); }).then(function () { toast("Approved & moved to expenses."); render(); }).catch(function (e) { toast(e.message); }); return; }
    ...
    if (a === "invgen") {
      ...
      pays.filter(function (p) { return d.pay && d.pay[p.id]; }).forEach(function (p) { items.push({ description: p.who + " — " + (p.contractorName || "") + (p.description ? " (" + p.description + ")" : ""), amount: parseFloat(p.amount || 0) }); });
      exps.filter(function (e) { return d.exp && d.exp[e.id]; }).forEach(function (e) { items.push({ description: (e.source === "Other" ? (e.sourceOther || "Other") : "American Express") + (e.description ? " — " + e.description : ""), amount: parseFloat(e.amount || 0) }); });
      fees.filter(function (fr) { return d.fee && d.fee[fr[0]]; }).forEach(function (fr) { months.forEach(function (m) { items.push({ description: fr[1] + " — " + monthLabel(m), amount: fr[2] }); }); });
      ...
      api("POST", "/api/invoices", { facilityId: fid, months: months, lineItems: items, total: total, expenseIds: expenseIds, payableIds: payableIds })
    ...
    if (a === "expadd") { S.expAdd = true; S.expDraft = {}; render(); return; }
    if (a === "expback") { S.expAdd = false; S.expDraft = {}; render(); return; }
    if (a === "expsrc") { S.expDraft = S.expDraft || {}; S.expDraft.source = el.getAttribute("data-v"); render(); return; }
    if (a === "expsave") { var ed = S.expDraft || {}; if (!(ed.amount || "").trim()) { toast("Enter an amount."); return; } api("POST", "/api/expenses", ed).then(function () { S.expDraft = {}; S.expAdd = false; return reloadExpenses(); }).then(function () { toast("Expense saved."); render(); }).catch(function (e) { toast(e.message); }); return; }
