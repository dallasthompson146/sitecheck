...
(function () {
  ...
  var S = {
    ...
    collections: [], week: "", collSub: "todo", collDraft: {},
  };
  ...
  function blobUrl(name) { if (!name) return ""; var t = token(); return "/api/blob/" + name + (t ? "?t=" + encodeURIComponent(t) : ""); }
  function toast(msg) {
  ...
  function compress(file) {
    return new Promise(function (resolve) {
      ...
      img.onload = function () {
        ...
        resolve({ id: uid(), kind: "image", url: c.toDataURL("image/jpeg", 0.72) });
      };
  ...
  function loadForRole() {
    if (S.auth.role === "admin" || S.auth.role === "superadmin") {
      ...
      calls.push(api("GET", "/api/collections"));
      ...
      return Promise.all(calls).then(function (r) { S.facilities = r[0]; S.users = r[1]; S.reports = r[2]; S.leads = r[3]; S.deposits = r[4]; S.reviews = r[5]; if (r[6]) { S.collections = r[6].collections || []; S.week = r[6].week; } if (r[7]) S.payables = r[7]; if (r[8]) S.recurring = r[8]; if (r[9]) S.expenses = r[9]; if (r[10]) S.billback = r[10]; if (r[11]) S.invoices = r[11]; if (r[12]) S.receipts = r[12]; });
    }
    if (S.auth.role === "employee") {
      return Promise.all([api("GET", "/api/facilities"), api("GET", "/api/leads"), api("GET", "/api/reviews"), api("GET", "/api/collections")])
        .then(function (r) { S.facilities = r[0]; S.leads = r[1]; S.reviews = r[2]; if (r[3]) { S.collections = r[3].collections || []; S.week = r[3].week; } });
    }
  ...
  function reloadCollections() { return api("GET", "/api/collections").then(function (r) { S.collections = r.collections || []; S.week = r.week; }); }
  ...
  function facilityEditor(f) {
    ...
    var ed = !!S.facHeadEdit;
    var nameCard = '<div class="card"><div class="between"><div class="dh" style="margin:0">Facility name &amp; address</div><button class="btn ' + (ed ? "btn-ok" : "btn-dark") + ' sm" data-a="facheadedit">' + (ed ? I("check") + " Done" : I("edit") + " Edit") + "</button></div>"
      + (ed
        ? '<div class="label">Facility name</div><input data-fac="name" value="' + esc(f.name || "") + '"><div class="label">Address</div><input data-fac="address" value="' + esc(f.address || "") + '"><p class="hint" style="margin:6px 0 0">Changes save automatically. Tap Done when finished.</p>'
        : '<div class="kv" style="margin-top:6px"><span style="color:#E0913C;font-weight:600">Name</span><span>' + esc(f.name || "\u2014") + '</span><span style="color:#E0913C;font-weight:600">Address</span><span>' + esc(f.address || "\u2014") + "</span></div><p class=\"hint\" style=\"margin:6px 0 0\">Tap Edit to change these.</p>")
      + '<div class="dh" style="margin-top:14px">Notes</div><textarea rows="3" data-fac="setupNotes" placeholder="Anything the team should know about this facility">' + esc(f.setupNotes || "") + "</textarea></div>";
    return '<button class="btn" data-a="backedit" style="margin-bottom:10px">' + I("arrow-left") + ' All facilities</button><h3 style="margin:0 0 2px">' + esc(f.name || "New facility") + '</h3><div class="muted" style="margin-bottom:10px">' + esc(f.address || "No address yet") + "</div>" + nameCard + nav + body;
  }
  function facPhotos(f, editable) {
    var photos = gA(f, "facilityPhotos");
    var thumbs = photos.map(function (p, i) { var src = p.blob ? blobUrl(p.blob) : (p.url || ""); return '<span style="position:relative;display:inline-block;margin:4px 4px 0 0"><img src="' + esc(src) + '" data-a="facphotoview" data-fid="' + f.id + '" data-k="' + i + '" style="height:84px;border-radius:8px;border:1px solid var(--line);cursor:pointer;object-fit:cover">' + (editable ? '<button class="icon-btn danger" data-a="facphotodel" data-id="' + (p.id || ("idx" + i)) + '" data-k="' + i + '" style="position:absolute;top:-6px;right:-6px;padding:2px">' + I("x") + "</button>" : "") + "</span>"; }).join("");
  ...
  function photoLightbox() {
    ...
    var p = photos[i], src = p.blob ? blobUrl(p.blob) : (p.url || "");
    return '<div class="lightbox" data-a="photoclose"><button class="lb-close" data-a="photoclose">' + I("x") + "</button>"
      ...
      + '<img src="' + esc(src) + '" class="lb-img" data-a="photonoop">'
  ...
  function facInfo(f) {
    ...
    var notesCard = '<div class="card"><div class="row" style="justify-content:space-between"><div style="font-weight:600">' + I("notes") + ' Internal notes</div><button class="btn btn-dark sm" data-a="noteadd">' + I("plus") + ' Add note</button></div><p class="hint" style="margin:6px 0 0">Shared reminders for the whole team \u2014 everyone can see these.</p>' + (notes.length ? '<div class="stack" style="margin-top:8px">' + notes.map(function (n) { return '<div class="litem"><textarea rows="2" style="flex:1" data-note="' + n.id + '">' + esc(n.text || "") + '</textarea><button class="icon-btn danger" data-a="notedel" data-id="' + n.id + '">' + I("trash") + "</button></div>"; }).join("") + "</div>" : '<div class="muted" style="font-size:13px;margin-top:8px">No notes yet.</div>') + "</div>";
    if (!ed) {
      ...
      return '<div class="between" style="margin-top:12px"><h4 style="margin:0">Facility information</h4><button class="btn btn-dark sm" data-a="facedit">' + I("edit") + ' Edit</button></div><p class="hint" style="margin:4px 0 10px">Tap Edit to change anything \u2014 fields aren\u2019t editable by accident.</p>'
        + '<div class="card"><div class="dh">Contact</div>' + ro("Name", f.name) + ro("Address", f.address) + ro("Phone", c.phone) + ro("Email", c.email) + ro("Website", c.website) + (c.googleReview ? '<div class="kv"><span style="color:#E0913C;font-weight:600">Google review link</span><span><a href="' + esc(c.googleReview) + '" target="_blank" rel="noopener">Open review page</a></span></div>' : ro("Google review link", "")) + ro("Secondary", c.secondary) + ro("Facebook", c.facebook) + ro("Instagram", c.instagram) + "</div>"
        ...
        + '<div class="card"><div class="dh">Previous information</div>' + ro("Previous facility name", c.prevName) + ro("Previous owner", c.prevOwner) + ro("Previous owner phone", c.prevOwnerPhone) + ro("Storage activities", yn(c.nonStorage)) + (c.nonStorage && c.nonStorageDesc ? '<div class="da">' + esc(c.nonStorageDesc) + "</div>" : "") + "</div>"
        + facPhotos(f, false)
        + notesCard;
    }
    return '<div class="between" style="margin-top:12px"><h4 style="margin:0">Edit information</h4><button class="btn btn-ok sm" data-a="facdone">' + I("check") + ' Done</button></div><div class="stack" style="margin-top:8px">'
      ...
      + '<div class="card"><div class="dh">Previous information</div>' + tf(f, "Previous storage facility name", "prevName") + tf(f, "Previous owner name", "prevOwner") + tf(f, "Previous owner phone", "prevOwnerPhone") + togF(f, "Has storage activities", "nonStorage") + (getPath(c, "nonStorage") ? taF(f, "Describe the storage activities", "nonStorageDesc") : "") + "</div>"
  ...
  function userEditor(w) {
    ...
    return '<button class="btn" data-a="backuser" style="margin-bottom:12px">' + I("arrow-left") + ' All logins</button><div class="stack">'
      ...
      + assignBlock
      + '<button class="btn btn-ok" data-a="saveuser" data-id="' + w.id + '" style="width:100%;justify-content:center">' + I("check") + ' Save this person</button>'
      + '<button class="btn btn-danger" data-a="deluser" data-id="' + w.id + '"' + (lockReason ? " disabled" : "") + ">" + I("trash") + " Remove this login</button>" + (lockReason ? '<div class="note">' + lockReason + "</div>" : "") + '<div class="saved">' + I("check") + ' Changes also save automatically as you type.</div></div>';
  }
  ...
  function employee() {
    ...
    var T = [["leads", "Lead tracking"], ["facinfo", "Facility info"], ["maint", "Maintenance"], ["collections", "Collections"], ["inventory", "Inventory"], ["contractors", "Contractors"]];
    ...
    if (tab === "collections") return tabs + collectionsView();
    if (tab === "facinfo") {
  ...
  function payCard(p, actions) {
    return '<div class="card"><div class="row" style="justify-content:space-between;align-items:flex-start"><div style="font-weight:700">' + esc(facName(p.facilityId)) + '</div><span class="muted" style="font-size:12px">Logged ' + fdt(p.createdAt) + '</span></div>'
      ...
      + (p.invoiceBlob ? '<a href="' + esc(blobUrl(p.invoiceBlob)) + '" target="_blank" rel="noopener" class="muted" style="font-size:12px;display:inline-block;margin-top:6px">' + I("paperclip") + " View attached invoice" + (/\.pdf$/i.test(p.invoiceBlob) ? " (PDF)" : "") + "</a>" : "")
      + (actions || "") + "</div>";
  }
  ...
  function buildReportPDF(rep) {
    if (!window.jspdf || !window.jspdf.jsPDF) { toast("PDF tool still loading, try again in a second."); return; }
    var f = S.facilities.find(function (x) { return x.id === rep.facilityId; }) || { name: "Facility", config: {} };
    var c = f.config || {}, d = rep.data || {}, filled = !!rep.submittedAt;
    var doc = new window.jspdf.jsPDF({ unit: "pt", format: "letter" });
    var W = doc.internal.pageSize.getWidth(), H = doc.internal.pageSize.getHeight(), M = 54, y = 0;
    var RED = [155, 28, 28], INK = [25, 25, 25], GREY = [110, 110, 110];
    function page() { doc.addPage(); y = M; }
    function need(h) { if (y + h > H - 70) page(); }
    // underlined section heading, like the paper form
    function sec(t) {
      need(34); y += 8;
      doc.setFont("times", "bold"); doc.setFontSize(12); doc.setTextColor(INK[0], INK[1], INK[2]);
      doc.text(t, M, y);
      var w = doc.getTextWidth(t);
      doc.setDrawColor(40); doc.setLineWidth(0.9); doc.line(M, y + 2.5, M + w, y + 2.5);
      y += 16;
    }
    // the tech's answer / the office's entry, in red like the form
    function val(t, indent) {
      var wrapped = doc.splitTextToSize(String(t), W - M * 2 - (indent || 0));
      need(15 * wrapped.length);
      doc.setFont("times", "bold"); doc.setFontSize(11.5); doc.setTextColor(RED[0], RED[1], RED[2]);
      doc.text(wrapped, M + (indent || 0), y); y += 14 * wrapped.length + 1;
    }
    function pair(left, right) {
      need(16);
      doc.setFont("times", "bold"); doc.setFontSize(11.5); doc.setTextColor(RED[0], RED[1], RED[2]);
      doc.text(String(left), M, y);
      if (right) doc.text(String(right), M + 190, y);
      y += 15;
    }
    function blanks(n) { // ruled lines for writing on when the form is printed empty
      for (var i = 0; i < n; i++) { need(20); doc.setDrawColor(190); doc.setLineWidth(0.5); doc.line(M, y + 6, W - M, y + 6); y += 20; }
    }
    function body(t) { var w = doc.splitTextToSize(String(t), W - M * 2); need(14 * w.length); doc.setFont("times", "normal"); doc.setFontSize(11); doc.setTextColor(INK[0], INK[1], INK[2]); doc.text(w, M, y); y += 13.5 * w.length; }

    /* ---- header, mirrors "Facility: ... / Check-in for Week of: ..." ---- */
    y = M;
    doc.setFont("times", "bold"); doc.setFontSize(13); doc.setTextColor(INK[0], INK[1], INK[2]);
    doc.text("Facility:", M, y);
    doc.setTextColor(RED[0], RED[1], RED[2]);
    doc.text(String(f.name || "").toUpperCase() + (rep.workerName ? " \u2014 " + rep.workerName.toUpperCase() : ""), M + 58, y);
    y += 18;
    doc.setTextColor(INK[0], INK[1], INK[2]);
    doc.text((rep.type === "audit" ? "Monthly Audit for:" : "Check-in for Week of:"), M, y);
    doc.setTextColor(RED[0], RED[1], RED[2]);
    doc.text(fday(rep.submittedAt || rep.createdAt || Date.now()), M + 150, y);
    y += 14;
    doc.setFont("times", "italic"); doc.setFontSize(9.5); doc.setTextColor(GREY[0], GREY[1], GREY[2]);
    doc.text(filled ? "Submitted " + fdt(rep.submittedAt) : "Blank form \u2014 fill in on site", M, y);
    y += 10;

    /* ---- TASKS FOR THE WEEK ---- */
    sec("TASKS FOR THE WEEK:");
    if ((c.weeklyTasks || []).length) {
      c.weeklyTasks.forEach(function (t) {
        var tr = (d.tasks || {})[t.id] || {};
        doc.setFont("times", "normal"); doc.setFontSize(11); doc.setTextColor(INK[0], INK[1], INK[2]);
        var tw = doc.splitTextToSize("\u2022 " + t.text, W - M * 2); need(14 * tw.length); doc.text(tw, M, y); y += 13.5 * tw.length;
        if (filled) val(tr.note ? tr.note : "\u2014", 14); else blanks(1);
      });
    } else { filled ? val("Nothing new this week.") : blanks(2); }

    /* ---- NEW LOCK OUTS ---- */
    sec("NEW LOCK OUTS FOR THIS WEEK:");
    if ((c.lockoutAdd || []).length) {
      c.lockoutAdd.forEach(function (u) {
        var a = (d.lockAdd || {})[u.id] || {};
        pair("Unit " + u.unit, filled ? ("L#" + (a.lockNo || "____") + "   " + (a.done ? "\u2713 locked" : "NOT DONE")) : "L#__________");
      });
    } else { filled ? val("None") : blanks(2); }

    /* ---- LOCKOUTS: LEAVE IN PLACE ---- */
    sec("LOCKOUTS: LEAVE IN PLACE:");
    if ((c.lockoutKeep || []).length) {
      c.lockoutKeep.forEach(function (u) { var a = (d.lockKeep || {})[u.id] || {}; pair("Unit " + u.unit, filled ? (a.done ? "\u2713 verified locked" : "NOT VERIFIED") : "\u2610 verified"); });
    } else { filled ? val("None") : blanks(2); }

    /* ---- LOCK OUTS: TO BE REMOVED ---- */
    sec("LOCK OUTS: TO BE REMOVED:");
    if ((c.lockoutRemove || []).length) {
      c.lockoutRemove.forEach(function (u) { var a = (d.lockRemove || {})[u.id] || {}; pair("Unit " + u.unit, filled ? (a.done ? "\u2713 removed" : "NOT REMOVED") : "\u2610 removed"); });
    } else { filled ? val("None") : blanks(2); }

    /* ---- UNITS NEEDING MAINTENANCE ---- */
    sec("UNITS NEEDING MAINTENANCE:");
    if ((c.maintenance || []).length) {
      c.maintenance.forEach(function (u) {
        pair("Unit " + u.unit, u.note || "");
        if (filled) val(((d.maintenance || {})[u.id] || {}).statement || "\u2014", 14); else blanks(1);
      });
    } else { filled ? val("None reported") : blanks(3); }

    /* ---- EMPTY UNITS (READY TO RENT) ---- */
    sec("EMPTY UNITS (READY TO RENT):");
    if ((c.vacant || []).length) {
      val(c.vacant.map(function (u) { return u.unit; }).join(", "));
      if (filled && d.vacantConfirmed === false && d.vacantExtra) val("Tech also found unlocked: " + d.vacantExtra, 0);
    } else { filled ? val("None on file") : blanks(2); }

    /* ---- VERIFICATION OF MOVEOUT ---- */
    sec("VERIFICATION OF MOVEOUT:");
    if ((c.vacated || []).length) {
      c.vacated.forEach(function (u) {
        var v = (d.vacated || {})[u.id] || {};
        if (filled) {
          pair("Unit " + u.unit, v.status === "clean" ? "\u2713 Clean / broom-swept" : v.status === "problem" ? "PROBLEM: " + (v.problem || "") : "\u2014");
          doc.setFont("times", "normal"); doc.setFontSize(10); doc.setTextColor(GREY[0], GREY[1], GREY[2]);
          need(14); doc.text("Door " + (v.door ? "\u2713" : "\u2717") + "   Latch " + (v.latch ? "\u2713" : "\u2717") + "   No intrusion " + (v.intrusion ? "\u2713" : "\u2717") + "   No water " + (v.water ? "\u2713" : "\u2717"), M + 14, y); y += 15;
        } else { pair("Unit " + u.unit, "\u2610 clean   \u2610 door   \u2610 latch   \u2610 no water"); }
      });
    } else { filled ? val("0") : blanks(2); }

    if ((c.auction || []).length) {
      sec("AUCTION UNITS:");
      c.auction.forEach(function (u) { var a = (d.auction || {})[u.id] || {}; pair("Unit " + u.unit, filled ? (a.note || (a.done ? "\u2713 done" : "not done")) : "\u2610 photographed & re-locked"); });
    }

    var g = d.grounds || {};
    sec("GROUNDS / GENERAL NOTES:");
    if (filled) { val(g.condition ? "Condition: " + g.condition : "\u2014"); if (g.note) val(g.note); if (rep.note) { doc.setFont("times", "italic"); doc.setFontSize(10); doc.setTextColor(GREY[0], GREY[1], GREY[2]); need(14); doc.text("Office note: " + rep.note, M, y); y += 15; } }
    else blanks(4);

    /* ---- standing instructions page (the directions sheet) ---- */
    page();
    doc.setFont("times", "bold"); doc.setFontSize(13); doc.setTextColor(INK[0], INK[1], INK[2]);
    doc.text("SITE CHECK INSTRUCTIONS", M, y); y += 22;
    var INSTR = [
      "Check recently vacated units to see if they are completely cleaned out; broom swept condition, and door and latch are operable.",
      "Report any units that are missing locks that are not on the vacated list. Open them to verify if there is anything in them. Take pics and send them to the group chat.",
      "Double lock any units on the lock out list.",
      "Remove any locks from the Remove Lock Out list.",
      "When you are cutting off locks for auction units, PLEASE DO NOT REMOVE ANYTHING FROM THE UNIT OR TOUCH ANYTHING. Take 2 or 3 pictures of the unit showing different angles and lock it back with one of our combination locks. Send the pictures back to the group chat.",
      "Pick up any trash from around the facility.",
      "Verify that the lawn is being kept up, and there are no visible weeds in the driveways or gravel areas.",
      "IMMEDIATELY report any units needing maintenance or any other problems that are worth noting regarding the property or customers.",
      "**REQUIRED** Take photos (at least 5) from different areas of the property for proof. And send them to the group chat."
    ];
    INSTR.forEach(function (t, i) {
      doc.setFont("times", "normal"); doc.setFontSize(11); doc.setTextColor(INK[0], INK[1], INK[2]);
      var w = doc.splitTextToSize((i + 1) + ".  " + t, W - M * 2 - 10);
      need(14 * w.length + 4); doc.text(w, M, y); y += 13.5 * w.length + 4;
    });
    y += 6;
    doc.setFont("times", "bold"); doc.setFontSize(11);
    body("**Make sure all unit doors are closed EVEN if they are empty.");
    y += 4;
    doc.setFont("times", "normal");
    body("If files are too big for the group chat, you can send those to our email address: Manager@storagedepotoh.com");

    y += 26; need(46);
    doc.setDrawColor(120); doc.setLineWidth(0.6);
    doc.line(M, y, M + 230, y); doc.line(W - M - 170, y, W - M, y);
    doc.setFont("times", "normal"); doc.setFontSize(9.5); doc.setTextColor(GREY[0], GREY[1], GREY[2]);
    doc.text("Technician signature", M, y + 12); doc.text("Date", W - M - 170, y + 12);

    var total = doc.internal.getNumberOfPages();
    for (var i = 1; i <= total; i++) {
      doc.setPage(i); doc.setFont("times", "normal"); doc.setFontSize(8.5); doc.setTextColor(150);
      doc.text((f.name || "") + "  \u00b7  " + (rep.type === "audit" ? "Monthly Audit" : "Weekly Check-in") + "  \u00b7  Page " + i + " of " + total, M, H - 34);
    }
    var safe = (f.name || "facility").replace(/[^A-Za-z0-9]+/g, "_");
    doc.save(safe + "_" + (rep.type === "audit" ? "audit" : "checkin") + "_" + new Date(rep.submittedAt || rep.createdAt || Date.now()).toISOString().slice(0, 10) + ".pdf");
  }
  ...
  function receiptCard(r) {
    return '<div class="card"><div class="between"><div style="font-weight:700">' + esc(r.name || "Receipt") + '</div><span class="muted" style="font-size:12px">' + fdt(r.createdAt) + "</span></div>"
      ...
      + '<div class="row" style="gap:8px;margin-top:8px">' + (r.blob ? '<a href="' + esc(blobUrl(r.blob)) + '" target="_blank" rel="noopener" class="btn sm">' + I("eye") + " View" + (/\.pdf$/i.test(r.blob) ? " (PDF)" : "") + "</a>" : '<span class="muted" style="font-size:12px">No file</span>') + '<button class="icon-btn danger" data-a="receiptdel" data-id="' + r.id + '">' + I("trash") + "</button></div></div>";
  }
  ...
  function manager() {
    ...
    var T = [["facilities", "Facilities"], ["facinfo", "Facility info"], ["leads", "Lead tracking"], ["maint", "Maintenance"], ["collections", "Collections"], ["inventory", "Inventory"], ["contractors", "Contractors"], ["checks", "Check tracking"], ["payables", "Payables"]];
    ...
    if (S.tab === "collections") { return tabs + collectionsView(); }
  ...
  function collectionsView() {
    var sub = S.collSub || "todo", isAdmin = S.auth.role === "admin" || S.auth.role === "superadmin";
    var DAYS = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"];
    var dayIdx = function (d) { var i = DAYS.indexOf(d || ""); return i === -1 ? 99 : i; };
    var colls = S.collections || [], wk = S.week || "";
    var doneThisWeek = function (fid) { return colls.find(function (c) { return c.facilityId === fid && c.week === wk; }); };
    var lastFor = function (fid) { var all = colls.filter(function (c) { return c.facilityId === fid; }).sort(function (a, b) { return (b.completedAt || 0) - (a.completedAt || 0); }); return all[0] || null; };
    // facilities with a collection day, sorted Monday -> Sunday
    var scheduled = S.facilities.filter(function (f) { return (f.config || {}).collectionDay; })
      .sort(function (a, b) { return dayIdx(a.config.collectionDay) - dayIdx(b.config.collectionDay) || (a.name || "").localeCompare(b.name || ""); });
    var tabs = [["todo", "This week\u2019s to-do"], ["history", "By facility"]];
    if (isAdmin) tabs.push(["setup", "Set up schedule"]);
    var nav = '<div class="tabs">' + tabs.map(function (t) { return '<button class="' + (sub === t[0] ? "on" : "") + '" data-a="collsub" data-k="' + t[0] + '">' + t[1] + "</button>"; }).join("") + "</div>";
    var body;
    if (sub === "setup") {
      body = '<p class="hint">Set which day of the week collections happen at each facility, and who handles them.</p>'
        + S.facilities.map(function (f) {
          var c = f.config || {};
          var dayOpts = '<option value="">\u2014 not scheduled \u2014</option>' + DAYS.map(function (d) { return '<option value="' + d + '"' + (c.collectionDay === d ? " selected" : "") + ">" + d + "</option>"; }).join("");
          var staff = (S.users || []).filter(function (u) { return u.role !== "worker"; });
          var empOpts = '<option value="">\u2014 unassigned \u2014</option>' + staff.map(function (u) { return '<option value="' + esc(u.name) + '"' + (c.collectionEmployee === u.name ? " selected" : "") + ">" + esc(u.name) + "</option>"; }).join("");
          return '<div class="card"><div style="font-weight:700">' + esc(f.name) + '</div><div class="two-col"><div><div class="label">Collection day</div><select data-collday="' + f.id + '">' + dayOpts + '</select></div><div><div class="label">Employee who handles it</div><select data-collemp="' + f.id + '">' + empOpts + "</select></div></div></div>";
        }).join("");
    } else if (sub === "history") {
      body = '<p class="hint">Most recent collection for each facility.</p>'
        + S.facilities.slice().sort(function (a, b) { return dayIdx((a.config || {}).collectionDay) - dayIdx((b.config || {}).collectionDay) || (a.name || "").localeCompare(b.name || ""); }).map(function (f) {
          var c = f.config || {}, last = lastFor(f.id), done = doneThisWeek(f.id);
          var hist = colls.filter(function (x) { return x.facilityId === f.id; }).sort(function (a, b) { return (b.completedAt || 0) - (a.completedAt || 0); }).slice(0, 8);
          var open = S.collOpen === f.id;
          return '<div class="card"><div class="between"><div><div style="font-weight:700">' + esc(f.name) + '</div><div class="muted" style="font-size:12px">' + (c.collectionDay ? esc(c.collectionDay) : "No day set") + (c.collectionEmployee ? " \u00b7 " + esc(c.collectionEmployee) : "") + "</div></div>" + (done ? '<span class="pill" style="background:#E1F0E9;color:var(--ok)">Completed this week</span>' : '<span class="pill" style="background:#FBEAE1;color:var(--hazard-d)">Not yet this week</span>') + "</div>"
            + '<div class="kv" style="margin-top:8px"><span style="color:#E0913C;font-weight:600">Most recent collection</span><span>' + (last ? fdt(last.completedAt) + " by " + esc(last.completedBy) : "\u2014") + "</span></div>"
            + (hist.length ? '<button class="btn sm" data-a="collopen" data-id="' + f.id + '" style="margin-top:8px">' + (open ? I("chevron-up") + " Hide history" : I("chevron-down") + " Show history") + "</button>" : "")
            + (open ? '<div class="dgrp" style="margin-top:8px">' + hist.map(function (h) { return '<div class="dline">' + fdt(h.completedAt) + " \u00b7 " + esc(h.completedBy) + (h.calledAll ? " \u00b7 called all customers" : "") + "</div>"; }).join("") + "</div>" : "")
            + "</div>";
        }).join("");
    } else {
      var pending = scheduled.filter(function (f) { return !doneThisWeek(f.id); });
      var doneList = scheduled.filter(function (f) { return doneThisWeek(f.id); });
      var card = function (f, isDone) {
        var c = f.config || {}, rec = doneThisWeek(f.id), d = (S.collDraft || {})[f.id] || {};
        var headBlock = '<div style="font-weight:600;font-size:17px;letter-spacing:.2px">' + esc(f.name) + "</div>"
          + '<div style="margin-top:10px"><div style="font-size:15px;font-weight:500;color:#C07C2E;letter-spacing:.3px">Typical Collection Date</div><div style="font-size:17px;font-weight:400;margin-top:1px">' + esc(c.collectionDay || "Not set") + "</div></div>"
          + '<div style="margin-top:10px"><div style="font-size:15px;font-weight:500;color:#C07C2E;letter-spacing:.3px">Typical employee who handles collections</div><div style="font-size:17px;font-weight:400;margin-top:1px">' + esc(c.collectionEmployee || "Unassigned") + "</div></div>";
        if (isDone) return '<div class="card" style="border-color:var(--ok)">' + headBlock + '<div class="row" style="justify-content:space-between;align-items:center;margin-top:10px"><span class="pill" style="background:#E1F0E9;color:var(--ok)">' + I("check") + ' Completed this week</span></div><div class="muted" style="font-size:12px;margin-top:6px">Done ' + fdt(rec.completedAt) + " by " + esc(rec.completedBy) + (rec.calledAll ? " \u00b7 reached out to every customer" : "") + "</div></div>";
        return '<div class="card">' + headBlock
          + '<button class="check full' + (d.calledAll ? " on" : "") + '" data-a="colltog" data-id="' + f.id + '" style="margin-top:12px"><span class="bx">' + (d.calledAll ? I("check") : "") + "</span>Reached out to every customer via call</button>"
          + '<button class="btn btn-ok" data-a="colldone" data-id="' + f.id + '" style="width:100%;justify-content:center;margin-top:10px">' + I("check") + " I completed collections this week</button></div>";
      };
      body = '<p class="hint">Confirm collections once a week for each facility. The week resets every Monday.</p>'
        + (scheduled.length ? ('<div class="dh">Still to do (' + pending.length + ")</div>" + (pending.length ? '<div class="stack" style="margin-top:6px">' + pending.map(function (f) { return card(f, false); }).join("") + "</div>" : '<div class="empty">All collections completed this week. Nice work.</div>')
          + (doneList.length ? '<div class="dh" style="margin-top:16px">Completed this week (' + doneList.length + ')</div><div class="stack" style="margin-top:6px">' + doneList.map(function (f) { return card(f, true); }).join("") + "</div>" : ""))
          : '<div class="empty">No facilities have a collection day set yet.' + (isAdmin ? " Use Set up schedule." : " Ask an admin to set one up.") + "</div>");
    }
    return "<h3>Collections</h3>" + nav + body;
  }
  ...
  function reportReview(rep) {
    ...
    var gal = function (arr, tag) { return arr && arr.length ? '<div class="gal">' + arr.filter(function (p) { return p.kind === "image" || p.blob || p.url; }).map(function (p, i) { var u = p.blob ? blobUrl(p.blob) : p.url; return '<a href="' + u + '" download="' + esc(f.name).replace(/[^A-Za-z0-9]+/g, "_") + "_" + (tag || "photo") + "_" + (i + 1) + '.jpg" title="Click to download"><img src="' + u + '"></a>'; }).join("") + '</div><div class="muted" style="font-size:12px;margin-top:4px">Tip: click a photo to download it.</div>' : ""; };
    ...
    var top = '<div class="wtop"><button class="btn" data-a="backopen">' + I("arrow-left") + ' Reports</button><button class="btn btn-dark sm" data-a="reppdf" data-id="' + rep.id + '">' + I("download") + " Download PDF</button>" + (rep.reviewed ? '<span class="pill" style="background:#E1F0E9;color:var(--ok)">' + I("check") + " Reviewed</span>" : '<span class="pill" style="background:#FBEAE1;color:var(--hazard-d)">Needs review</span>') + "</div>";
  ...
  function photoField(label, min, arr, scope) {
    ...
    return '<div class="label">' + I("camera") + " " + label + '<span class="count ' + (ok ? "ok" : "need") + '">' + arr.length + (min ? " / " + min + " min" : "") + "</span></div><div class=\"photos\">"
      + arr.map(function (p) { return '<div class="thumb"><img src="' + (p.blob ? blobUrl(p.blob) : p.url) + '"><button class="xx" data-a="delphoto" data-scope="' + scope + '" data-id="' + p.id + '">' + I("x") + "</button></div>"; }).join("")
      + '<label class="addph">' + I("plus") + 'Add photo<input type="file" accept="image/*" multiple style="display:none" data-add="' + scope + '"></label></div>';
  }
  ...
  function worker() {
    ...
    var head = '<div class="wo"><div class="between"><div><div class="eyebrow">Work order</div><h4>' + esc(f.name) + '</h4></div><div class="row" style="gap:6px;flex-wrap:wrap"><span class="chip" style="display:inline-flex;align-items:center;gap:4px">' + I("calendar") + " " + (f.checkInDay || "—") + "</span></div></div>"
      + '<div class="wprog"><div class="wprog-bar"><span style="width:' + pct + '%"></span></div><div class="wprog-txt">Step ' + (S.step + 1) + " of " + STEPS.length + " \u00b7 " + esc(sk) + "</div></div>"
      + pips + "</div>";
  ...
  document.addEventListener("change", function (ev) {
    ...
    if (t.getAttribute("data-collday") !== null) { var cf = S.facilities.find(function (x) { return x.id === t.getAttribute("data-collday"); }); if (cf) { cf.config.collectionDay = t.value; putFacility(cf).then(render).catch(function (e) { toast(e.message); }); } return; }
    if (t.getAttribute("data-collemp") !== null) { var cf2 = S.facilities.find(function (x) { return x.id === t.getAttribute("data-collemp"); }); if (cf2) { cf2.config.collectionEmployee = t.value; putFacility(cf2).then(render).catch(function (e) { toast(e.message); }); } return; }
  ...
  /* ---------- clicks ---------- */
  var CLICK_GUARD = {};
  // Actions that write to the server — ignore a repeat click within 1.5s so a double tap can't submit twice.
  var GUARDED = { submit: 1, savelead: 1, savefu: 1, depfinal: 1, mtsubmit: 1, expsave: 1, paysave: 1, invgen: 1, reviewsave: 1, markreviewed: 1, payapprove: 1, payreject: 1, depsettle: 1, saveuser: 1, adduser: 1, sendreport: 1, sendaudit: 1, bbsave: 1 };
  document.addEventListener("click", function (ev) {
    ...
    if (GUARDED[a]) {
      var gk = a + "|" + (id || "") + "|" + (k || ""), now = Date.now();
      if (CLICK_GUARD[gk] && now - CLICK_GUARD[gk] < 1500) { ev.preventDefault(); return; }
      CLICK_GUARD[gk] = now;
    }
    ...
    /* facilities */
    if (a === "addfac") { api("POST", "/api/facilities", {}).then(function (f) { S.facilities.push(f); S.editId = f.id; S.infoMode = false; S.facView = "sitecheck"; S.facEdit = true; S.facHeadEdit = true; S.subOpen = null; render(); }).catch(function (e) { toast(e.message); }); return; }
    if (a === "editfac") { S.editId = id; S.infoMode = false; S.facView = "sitecheck"; S.facEdit = false; S.facHeadEdit = false; S.subOpen = null; render(); return; }
    ...
    if (a === "saveuser") { flushSave(); var su = S.users.find(function (x) { return x.id === id; }); if (!su) { S.userId = null; render(); return; } if (!(su.name || "").trim()) { toast("Enter a name."); return; } if (!(su.username || "").trim()) { toast("Enter a username."); return; } api("GET", "/api/users").then(function (us) { S.users = us; S.userId = null; toast("Saved."); render(); }).catch(function (e) { toast(e.message); }); return; }
    ...
    if (a === "reppdf") { var rp = (S.reports || []).find(function (x) { return x.id === id; }) || (S.subOpen && S.subOpen.id === id ? S.subOpen : null); if (!rp) { toast("Report not found."); return; } buildReportPDF(rp); return; }
    if (a === "facheadedit") { S.facHeadEdit = !S.facHeadEdit; if (!S.facHeadEdit) flushSave(); render(); return; }
    if (a === "collsub") { S.collSub = k; render(); return; }
    if (a === "collopen") { S.collOpen = (S.collOpen === id ? null : id); render(); return; }
    if (a === "colltog") { S.collDraft = S.collDraft || {}; S.collDraft[id] = S.collDraft[id] || {}; S.collDraft[id].calledAll = !S.collDraft[id].calledAll; render(); return; }
    if (a === "colldone") {
      var cd = ((S.collDraft || {})[id]) || {};
      api("POST", "/api/collections", { facilityId: id, calledAll: !!cd.calledAll })
        .then(function () { if (S.collDraft) delete S.collDraft[id]; return reloadCollections(); })
        .then(function () { toast("Collections completed."); render(); })
