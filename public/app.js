...
(function () {
  ...
  function loadForRole() {
    if (S.auth.role === "admin" || S.auth.role === "superadmin") {
      ...
      if (S.auth.role === "superadmin") calls.push(api("GET", "/api/payables"), api("GET", "/api/recurring"));
      return Promise.all(calls).then(function (r) { S.facilities = r[0]; S.users = r[1]; S.reports = r[2]; S.leads = r[3]; if (r[4]) S.payables = r[4]; if (r[5]) S.recurring = r[5]; });
    }
  ...
  function payablesView() {
    ...
    var st = [["pending", "Pending (" + pend + ")"], ["new", "New payable"], ["expenses", "Expenses"], ["billedback", "Billed back"], ["recurring", "Recurring"]];
    var nav = '<div class="tabs">' + st.map(function (t) { return '<button class="' + (sub === t[0] ? "on" : "") + '" data-a="paysub" data-k="' + t[0] + '">' + t[1] + "</button>"; }).join("") + "</div>";
    var body = sub === "new" ? payableForm() : sub === "expenses" ? payExpenses() : sub === "billedback" ? payBilledBack() : sub === "recurring" ? recurringView() : payPending();
