...
(function () {
  ...
  function facilityEditor(f) {
    ...
    return '<button class="btn" data-a="backedit" style="margin-bottom:12px">' + I("arrow-left") + ' All facilities</button><div class="stack">'
      ...
      + grp("Master unit list (for monthly audits)", "list-numbers", '<p class="hint" style="margin:0 0 8px">List every unit at this facility. On an audit, anything here that is not in the Vacant list above is treated as occupied and the tech confirms a customer lock is on it.</p>' + listEditor(f.config.units || [], "units", [{ k: "unit", ph: "Unit #", unit: 1 }], "Add unit"))
  ...
  function reportsHome() {
    ...
    var auditBanner = dueAudits.length ? ('<div class="card" style="border-color:var(--hazard-d);background:#FBEAE1;margin-bottom:14px"><div style="font-weight:700;color:var(--hazard-d)">' + I("alert-triangle") + ' Monthly audit due (' + dueAudits.length + ')</div><p class="hint" style="margin:6px 0 10px">A more in-depth monthly check. Send it to the facility\u2019s techs, or disregard if it\u2019s already been done.</p>' + dueAudits.map(function (f) { return '<div class="row" style="justify-content:space-between;align-items:center;gap:8px;padding:6px 0;border-top:1px solid var(--line)"><span style="font-weight:600">' + esc(f.name) + '</span><span class="row" style="gap:6px"><button class="btn btn-dark sm" data-a="sendaudit" data-id="' + f.id + '">' + I("send") + ' Send audit</button><button class="btn sm" data-a="auditdone" data-id="' + f.id + '">Disregard</button></span></div>'; }).join("") + '</div>') : "";
  ...
  document.addEventListener("click", function (ev) {
    ...
    if (a === "sendaudit") { api("POST", "/api/reports", { facilityId: id, type: "audit", note: "Monthly audit" }).then(function () { var ff = S.facilities.find(function (x) { return x.id === id; }); if (ff) { ff.config.lastAuditAt = Date.now(); ff.config.updatedAt = Date.now(); } return reloadReports(); }).then(function () { toast("Audit sent to the facility's techs."); render(); }).catch(function (e) { toast(e.message); }); return; }
