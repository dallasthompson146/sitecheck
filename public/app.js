...
(function () {
  ...
  function facilityEditor(f) {
    ...
    var nameCard = '<div class="card"><div class="dh">Facility name &amp; address</div><div class="label">Facility name</div><input data-fac="name" value="' + esc(f.name || "") + '"><div class="label">Address</div><input data-fac="address" value="' + esc(f.address || "") + '"><p class="hint" style="margin:6px 0 0">Changes save automatically. Full contact details live on the Facility info tab.</p></div>';
  ...
  function collectionsView() {
    ...
    if (sub === "setup") {
      ...
      body = '<p class="hint">Most recent collection for each facility.</p>'
        + S.facilities.map(function (f) {
          ...
          return '<div class="card"><div class="between"><div><div style="font-weight:700">' + esc(f.name) + '</div><div class="muted" style="font-size:12px">' + (c.collectionDay ? esc(c.collectionDay) : "No day set") + (c.collectionEmployee ? " \u00b7 " + esc(c.collectionEmployee) : "") + "</div></div>" + (done ? '<span class="pill" style="background:#E1F0E9;color:var(--ok)">Done this week</span>' : '<span class="pill" style="background:#FBEAE1;color:var(--hazard-d)">Not yet this week</span>') + "</div>"
            ...
            + (open ? '<div class="dgrp" style="margin-top:8px">' + hist.map(function (h) { return '<div class="dline">' + fdt(h.completedAt) + " \u00b7 " + esc(h.completedBy) + (h.amount ? " \u00b7 " + esc(h.amount) : "") + (h.notes ? '<div class="da">' + esc(h.notes) + "</div>" : "") + (isAdmin ? ' <button class="icon-btn danger" data-a="colldel" data-id="' + h.id + '" style="padding:0 4px">' + I("trash") + "</button>" : "") + "</div>"; }).join("") + "</div>" : "")
            + "</div>";
        }).join("");
    } else {
      var mine = S.facilities.filter(function (f) { return (f.config || {}).collectionDay; });
      var pending = mine.filter(function (f) { return !doneThisWeek(f.id); });
      var done = mine.filter(function (f) { return doneThisWeek(f.id); });
      var card = function (f, isDone) {
        var c = f.config || {}, rec = doneThisWeek(f.id), d = (S.collDraft || {})[f.id] || {};
        if (isDone) return '<div class="card" style="border-color:var(--ok)"><div class="between"><div><div style="font-weight:700">' + esc(f.name) + '</div><div class="muted" style="font-size:12px">' + esc(c.collectionDay || "") + (c.collectionEmployee ? " \u00b7 " + esc(c.collectionEmployee) : "") + '</div></div><span class="pill" style="background:#E1F0E9;color:var(--ok)">' + I("check") + " Confirmed</span></div><div class=\"muted\" style=\"font-size:12px;margin-top:6px\">Done " + fdt(rec.completedAt) + " by " + esc(rec.completedBy) + (rec.amount ? " \u00b7 " + esc(rec.amount) : "") + "</div>" + (rec.notes ? '<div class="da">' + esc(rec.notes) + "</div>" : "") + "</div>";
        return '<div class="card"><div class="between"><div><div style="font-weight:700">' + esc(f.name) + '</div><div class="muted" style="font-size:12px">Collections: ' + esc(c.collectionDay || "") + (c.collectionEmployee ? " \u00b7 " + esc(c.collectionEmployee) : "") + "</div></div></div>"
          + '<div class="two-col" style="margin-top:8px"><div><div class="label">Amount collected (optional)</div><input data-coll="' + f.id + '|amount" value="' + esc(d.amount || "") + '" placeholder="$"></div><div><div class="label">Notes (optional)</div><input data-coll="' + f.id + '|notes" value="' + esc(d.notes || "") + '"></div></div>'
      ...
      body = '<p class="hint">Confirm collections once a week for each facility. The week resets every Monday.</p>'
        + (mine.length ? ('<div class="dh">Still to do (' + pending.length + ")</div>" + (pending.length ? '<div class="stack" style="margin-top:6px">' + pending.map(function (f) { return card(f, false); }).join("") + "</div>" : '<div class="empty">All collections confirmed this week. Nice work.</div>')
          + (done.length ? '<div class="dh" style="margin-top:16px">Confirmed this week (' + done.length + ')</div><div class="stack" style="margin-top:6px">' + done.map(function (f) { return card(f, true); }).join("") + "</div>" : ""))
  ...
  document.addEventListener("input", function (ev) {
    ...
    if (coll) { var cp = coll.split("|"); S.collDraft = S.collDraft || {}; S.collDraft[cp[0]] = S.collDraft[cp[0]] || {}; S.collDraft[cp[0]][cp[1]] = v; return; }
  ...
  document.addEventListener("click", function (ev) {
    ...
    /* facilities */
    if (a === "addfac") { api("POST", "/api/facilities", {}).then(function (f) { S.facilities.push(f); S.editId = f.id; S.infoMode = false; S.facView = "sitecheck"; S.facEdit = true; S.subOpen = null; render(); }).catch(function (e) { toast(e.message); }); return; }
    if (a === "editfac") { S.editId = id; S.infoMode = false; S.facView = "sitecheck"; S.facEdit = false; S.subOpen = null; render(); return; }
    ...
    if (a === "colldone") {
      ...
      api("POST", "/api/collections", { facilityId: id, amount: cd.amount || "", notes: cd.notes || "" })
        .then(function () { if (S.collDraft) delete S.collDraft[id]; return reloadCollections(); })
        .then(function () { toast("Collections confirmed."); render(); })
        .catch(function (e) { toast(e.message); });
      return;
    }
    if (a === "colldel") { if (!confirm("Remove this collection record?")) return; api("DELETE", "/api/collections/" + id).then(reloadCollections).then(render).catch(function (e) { toast(e.message); }); return; }
