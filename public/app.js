...
(function () {
  ...
  function loadForRole() {
    if (S.auth.role === "admin" || S.auth.role === "superadmin") {
      var calls = [api("GET", "/api/facilities"), api("GET", "/api/users"), api("GET", "/api/reports"), api("GET", "/api/leads")];
      ...
      return Promise.all(calls).then(function (r) { S.facilities = r[0]; S.users = r[1]; S.reports = r[2]; S.leads = r[3]; if (r[4]) S.payables = r[4]; if (r[5]) S.recurring = r[5]; if (r[6]) S.expenses = r[6]; if (r[7]) S.billback = r[7]; if (r[8]) S.invoices = r[8]; });
    }
  ...
  function manager() {
    ...
    var T = [["facilities", "Facilities"], ["logins", "Logins"], ["leads", "Lead tracking"], ["reports", "Reports"], ["maint", "Maintenance"], ["inventory", "Inventory"], ["contractors", "Contractors"], ["payables", "Payables"]];
