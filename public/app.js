...
(function () {
  ...
  function fuScheduleCard(ref, selKey, action, reasonHtml) {
    var mi = ref && !ref.moveInUnknown && ref.estMoveIn ? Date.parse(ref.estMoveIn + "T12:00:00") : null;
    var now = Date.now(), DAYms = 864e5;
    var opts = [["", "Standard schedule (recommended)"], ["h4", "Follow up with customer in 4 hours"], ["d1", "Follow up with customer in a day"]];
    if (!mi || mi - now > 7 * DAYms) opts.push(["w1", "Follow up with customer in a week"]);
    if (mi && mi - DAYms > now) opts.push(["preMoveIn", "Follow up with customer a day before move-in"]);
    if (mi && mi > now) opts.push(["moveInDay", "Follow up with customer the day of move-in"]);
    if (!mi || mi - now > 30 * DAYms) opts.push(["m1", "Follow up with customer in a month"]);
    var sel = selKey || "";
    var btns = opts.map(function (o) { return '<button class="check full' + (sel === o[0] ? " on" : "") + '" data-a="' + action + '" data-v="' + o[0] + '"><span class="bx">' + (sel === o[0] ? I("check") : "") + "</span>" + o[1] + "</button>"; }).join("");
    return '<div class="card"><div class="label">When should the next follow-up be?</div><p class="hint" style="margin:0 0 8px"><b>* Only change this from the standard schedule if something the customer said in the conversation means they should be followed up with at a later date.</b></p>' + btns + '<p class="hint" style="margin:8px 0 0">Choosing a later start removes the earlier follow-ups; every standard follow-up after that stays the same.</p>' + (sel ? reasonHtml : "") + "</div>";
  }
  function leadAddForm() {
    ...
    return '<button class="btn" data-a="leadback" style="margin-bottom:12px">' + I("arrow-left") + ' Lead tracking</button><div class="stack">'
      ...
      + fuScheduleCard(d, d.firstFollowup, "leadfu", '<div class="label">Reason for changing the follow-up date (required)</div><textarea rows="2" data-lead="firstFollowupReason" placeholder="What did the customer say that means a later follow-up?">' + esc(d.firstFollowupReason || "") + "</textarea>")
  ...
  function followupForm(l, f) {
    ...
    return '<button class="btn" data-a="fuback" style="margin-bottom:12px">' + I("arrow-left") + ' Lead</button><div class="stack">'
      ...
      + fuScheduleCard(l, d.nextFollowup, "funext", '<div class="label">Reason for changing the follow-up date (required)</div><textarea rows="2" data-fu="nextFollowupReason" placeholder="What did the customer say that means a later follow-up?">' + esc(d.nextFollowupReason || "") + "</textarea>")
  ...
  document.addEventListener("click", function (ev) {
    ...
    if (a === "leadfu") { S.leadDraft = S.leadDraft || {}; S.leadDraft.firstFollowup = el.getAttribute("data-v"); render(); return; }
    if (a === "funext") { S.fuDraft = S.fuDraft || {}; S.fuDraft.nextFollowup = el.getAttribute("data-v"); render(); return; }
