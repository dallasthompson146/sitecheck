...
(function () {
  ...
  var S = {
    ...
    userId: null, leads: [], leadTab: "warm", leadOpen: null, leadAdd: false, leadDraft: {}, fuOpen: null, fuDraft: {},
  };
  ...
  function leadTog(label, key, val) { return '<div class="qrow"><span>' + label + '</span><div class="yn"><button class="' + (val === true ? "on yes" : "") + '" data-a="leadtog" data-k="' + key + '" data-v="1">Yes</button><button class="' + (val === false ? "on no" : "") + '" data-a="leadtog" data-k="' + key + '" data-v="0">No</button></div></div>'; }
  function leadAddForm() {
    ...
    var resv = d.kind === "reservation" ? '<div class="card">' + leadTog("Was the reservation put into Cubby?", "inCubby", d.inCubby) + leadTog("Was it a SpareFoot reservation?", "spareFoot", d.spareFoot) + leadTog("Did you inform the customer of any discounts or promotions in the initial conversation?", "informedDiscounts", d.informedDiscounts) + "</div>" : "";
    return '<button class="btn" data-a="leadback" style="margin-bottom:12px">' + I("arrow-left") + ' Lead tracking</button><div class="stack">'
      ...
      + '<div class="card"><div class="label">Facility</div><select data-lead="facilityId">' + facOpts + '</select><div class="label">Customer number</div><input data-lead="phone" value="' + esc(d.phone || "") + '"><div class="label">Secondary number (leave blank if unknown)</div><input data-lead="phone2" value="' + esc(d.phone2 || "") + '"><div class="label">Email (leave blank if unknown)</div><input data-lead="email" value="' + esc(d.email || "") + '"></div>'
  ...
  function fuT(label, key, val) { return '<div class="qrow"><span>' + label + '</span><div class="yn"><button class="' + (val === true ? "on yes" : "") + '" data-a="futog" data-k="' + key + '" data-v="1">Yes</button><button class="' + (val === false ? "on no" : "") + '" data-a="futog" data-k="' + key + '" data-v="0">No</button></div></div>'; }
  function leadsView() {
    ...
    var tabs = '<div class="tabs">' + bt.map(function (t) { return '<button class="' + (tab === t[0] ? "on" : "") + '" data-a="leadtab" data-k="' + t[0] + '">' + esc(t[1]) + " (" + counts[t[0]] + ")</button>"; }).join("") + "</div>";
    var content;
  ...
  document.addEventListener("click", function (ev) {
    ...
    if (a === "leadback") { S.leadAdd = false; S.leadDraft = {}; render(); return; }
    ...
    if (a === "sendreport") { var fid = S.editId || S.newReportFac || (S.facilities[0] || {}).id; if (!fid) { toast("Add a facility first."); return; } api("POST", "/api/reports", { facilityId: fid, note: S.newReportNote || "" }).then(function () { S.newReportNote = ""; return reloadReports(); }).then(function () { toast("Report request sent."); render(); }).catch(function (e) { toast(e.message); }); return; }
