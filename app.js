...
(function () {
  ...
  function moveInTracker() {
    ...
    var revs = S.reviews || [];
    var revBlock = "";
    var byEmp = {}; revs.forEach(function (r) { (byEmp[r.employee || "Unknown"] = byEmp[r.employee || "Unknown"] || []).push(r); });
  ...
  function reviewAddForm() {
    ...
    return '<button class="btn" data-a="reviewback" style="margin-bottom:10px">' + I("arrow-left") + ' Lead tracking</button><h3>Add Google review</h3><div class="card">'
      ...
      + '<div class="label">Which employee got the review?</div><input data-rv="employee" value="' + esc(d.employee || "") + '" placeholder="Employee name">'
  ...
  function depNew() {
    ...
    var rows = d.entries.map(function (e) {
      return '<div class="card" style="padding:8px"><div class="row" style="gap:6px;flex-wrap:wrap;align-items:flex-end">'
        + '<div style="flex:2;min-width:120px"><div class="label" style="margin:0 0 2px">Facility</div><select data-de="' + e.id + '|facilityId">' + facOpts(e.facilityId) + "</select></div>"
        + '<div style="flex:2;min-width:110px"><div class="label" style="margin:0 0 2px">Customer</div><input data-de="' + e.id + '|customer" value="' + esc(e.customer || "") + '"></div>'
        + '<div style="flex:2;min-width:110px"><div class="label" style="margin:0 0 2px">What it\u2019s for</div><input data-de="' + e.id + '|forWhat" value="' + esc(e.forWhat || "") + '"></div>'
        + '<div style="width:74px"><div class="label" style="margin:0 0 2px">Type</div><select data-de="' + e.id + '|type"><option' + (e.type !== "MO" ? " selected" : "") + ">CK</option><option" + (e.type === "MO" ? " selected" : "") + ">MO</option></select></div>"
        + '<div style="width:90px"><div class="label" style="margin:0 0 2px">Check #</div><input data-de="' + e.id + '|checkNo" value="' + esc(e.checkNo || "") + '"></div>'
        + '<div style="width:90px"><div class="label" style="margin:0 0 2px">Amount</div><input data-de="' + e.id + '|amount" value="' + esc(e.amount || "") + '" placeholder="$"></div>'
        + '<button class="icon-btn danger" data-a="depdel" data-id="' + e.id + '">' + I("trash") + "</button>"
        + "</div></div>";
    }).join("");
  ...
  function receiptsView() {
    var rs = S.receipts || [];
    var upload = '<div class="card"><div style="font-weight:600">' + I("upload") + ' Upload a receipt or invoice</div><p class="hint" style="margin:6px 0 8px">Add a photo or PDF directly. When a payable with an attached invoice is approved, it also shows up here automatically.</p><label class="addph" style="display:inline-flex">' + I("paperclip") + 'Attach file<input type="file" accept="image/*,application/pdf" style="display:none" data-rcfile></label></div>';
    var list = rs.length ? '<div class="stack">' + rs.map(function (r) {
      return '<div class="card"><div class="between"><div style="font-weight:700">' + esc(r.name || "Receipt") + '</div><span class="muted" style="font-size:12px">' + fdt(r.createdAt) + "</span></div>"
        + (r.facilityId || r.source ? '<div class="kv" style="margin-top:6px">' + (r.facilityId ? "<span>Facility</span><span>" + esc(facName(r.facilityId)) + "</span>" : "") + (r.source ? "<span>Source</span><span>" + esc(r.source) + "</span>" : "") + "</div>" : "")
    ...
    }).join("") + "</div>" : '<div class="empty">No receipts yet.</div>';
    return upload + '<div class="dh" style="margin-top:14px">Receipts &amp; invoices (' + rs.length + ")</div>" + list;
  }
  function payablesView() {
    ...
    var st = [["pending", "Pending (" + pend + ")"], ["new", "New payable"], ["expenses", "Expenses"], ["general", "General"], ["billedback", "Billed back"], ["billback", "Bill-back config"], ["recurring", "Recurring"], ["invoices", "Invoices"], ["paidinvoices", "Paid invoices"], ["receipts", "Receipts"]];
    var nav = '<div class="tabs">' + st.map(function (t) { return '<button class="' + (sub === t[0] ? "on" : "") + '" data-a="paysub" data-k="' + t[0] + '">' + t[1] + "</button>"; }).join("") + "</div>";
    var body = sub === "new" ? payableForm() : sub === "expenses" ? payExpenses() : sub === "general" ? generalExpenses() : sub === "billedback" ? payBilledBack() : sub === "billback" ? billbackConfig() : sub === "recurring" ? recurringView() : sub === "configure" ? configureBillBack() : sub === "invoices" ? invoicesView() : sub === "paidinvoices" ? paidInvoices() : sub === "receipts" ? receiptsView() : payPending();
  ...
  document.addEventListener("click", function (ev) {
    ...
    if (a === "reviewsave") { var rv = S.reviewDraft || {}; if (!(rv.customer || "").trim()) { toast("Enter the customer name."); return; } if (!(rv.employee || "").trim()) { toast("Enter which employee got the review."); return; } api("POST", "/api/reviews", rv).then(function () { S.reviewDraft = {}; S.reviewAdd = false; S.leadTab = "tracker"; return reloadReviews(); }).then(function () { toast("Google review logged."); render(); }).catch(function (e) { toast(e.message); }); return; }
