...
(function () {
  ...
  function buildReportPDF(rep) {
    ...
    var c = f.config || {}, d = rep.data || {};
    var doc = new window.jspdf.jsPDF({ unit: "pt", format: "letter" });
    var W = doc.internal.pageSize.getWidth(), M = 46, y = 0;
    var yn = function (v) { return v === true ? "Yes" : v === false ? "No" : "\u2014"; };
    var done = function (b) { return b ? "Done" : "Not done"; };
    function page() { doc.addPage(); y = M; }
    function need(h) { if (y + h > doc.internal.pageSize.getHeight() - M) page(); }
    function h1(t) { need(30); doc.setFont("helvetica", "bold"); doc.setFontSize(15); doc.setTextColor(20); doc.text(t, M, y); y += 20; }
    function h2(t) { need(26); doc.setFillColor(238, 241, 244); doc.rect(M, y - 11, W - M * 2, 20, "F"); doc.setFont("helvetica", "bold"); doc.setFontSize(11); doc.setTextColor(30); doc.text(t, M + 6, y + 3); y += 26; }
    function line(label, val) {
      var txt = (val === undefined || val === null || val === "") ? "\u2014" : String(val);
      var wrapped = doc.splitTextToSize(txt, W - M * 2 - 130);
      need(14 * wrapped.length + 6);
      doc.setFont("helvetica", "bold"); doc.setFontSize(9.5); doc.setTextColor(90); doc.text(String(label), M, y);
      doc.setFont("helvetica", "normal"); doc.setFontSize(10); doc.setTextColor(20); doc.text(wrapped, M + 130, y);
      y += 13 * wrapped.length + 3;
    }
    function note(t) { var w = doc.splitTextToSize(String(t || "\u2014"), W - M * 2); need(13 * w.length + 4); doc.setFont("helvetica", "normal"); doc.setFontSize(10); doc.setTextColor(20); doc.text(w, M, y); y += 12 * w.length + 4; }

    y = M;
    doc.setFont("helvetica", "bold"); doc.setFontSize(19); doc.text(rep.type === "audit" ? "Monthly Audit" : "Site Check Report", M, y); y += 22;
    doc.setFont("helvetica", "normal"); doc.setFontSize(11); doc.setTextColor(70);
    doc.text(f.name + (f.address ? "  \u00b7  " + f.address : ""), M, y); y += 16;
    doc.setFontSize(9.5); doc.setTextColor(110);
    doc.text("Technician: " + (rep.workerName || "\u2014") + "     Submitted: " + (rep.submittedAt ? fdt(rep.submittedAt) : "\u2014") + "     Printed: " + fdt(Date.now()), M, y); y += 8;
    doc.setDrawColor(210); doc.line(M, y, W - M, y); y += 20; doc.setTextColor(20);

    if ((c.weeklyTasks || []).length) { h2("Weekly tasks"); c.weeklyTasks.forEach(function (t) { var tr = (d.tasks || {})[t.id] || {}; line(t.text, tr.note); }); }
    var hasLock = (c.lockoutAdd || []).length || (c.lockoutRemove || []).length || (c.lockoutKeep || []).length;
    if (hasLock) {
      h2("Lockout");
      (c.lockoutAdd || []).forEach(function (u) { var a = (d.lockAdd || {})[u.id] || {}; line("Add \u2014 Unit " + u.unit, "Lock #" + (a.lockNo || "____") + " \u00b7 " + done(a.done)); });
      (c.lockoutRemove || []).forEach(function (u) { line("Remove \u2014 Unit " + u.unit, done(((d.lockRemove || {})[u.id] || {}).done)); });
      (c.lockoutKeep || []).forEach(function (u) { line("Leave \u2014 Unit " + u.unit, done(((d.lockKeep || {})[u.id] || {}).done)); });
    }
    if ((c.maintenance || []).length) { h2("Maintenance"); c.maintenance.forEach(function (u) { line("Unit " + u.unit + " \u2014 " + (u.note || ""), ((d.maintenance || {})[u.id] || {}).statement); }); }
    if ((c.vacated || []).length) {
      h2("Recently vacated");
      c.vacated.forEach(function (u) {
        var v = (d.vacated || {})[u.id] || {};
        var st = v.status === "clean" ? "Clean / broom-swept" : v.status === "problem" ? "PROBLEM: " + (v.problem || "") : "\u2014";
        line("Unit " + u.unit, st);
        line("", "Door: " + yn(v.door) + " \u00b7 Latch: " + yn(v.latch) + " \u00b7 No intrusion: " + yn(v.intrusion) + " \u00b7 No water: " + yn(v.water));
      });
    }
    if (rep.type === "audit" && (c.vacant || []).length) {
      h2("Vacant units (audit)");
      c.vacant.forEach(function (u) { var v = (d.vacantAudit || {})[u.id] || {}; line("Unit " + u.unit, (v.status === "clean" ? "Clean" : v.status === "problem" ? "PROBLEM: " + (v.problem || "") : "\u2014") + " \u00b7 Door " + yn(v.door) + " \u00b7 Latch " + yn(v.latch)); });
    } else if ((c.vacant || []).length) {
      h2("Vacant");
      line("Office list", c.vacant.map(function (u) { return "Unit " + u.unit; }).join(", "));
      line("Tech found", d.vacantConfirmed === false ? "Other unlocked units: " + (d.vacantExtra || "") : d.vacantConfirmed ? "Confirmed only these are vacant" : "\u2014");
    }
    if (rep.type === "audit") { var occ = occupiedUnits(c); if (occ.length) { h2("Occupied units (audit)"); occ.forEach(function (u) { var o = (d.occupied || {})[u.id] || {}; line("Unit " + u.unit, o.hasLock === true ? "Customer lock on" : o.hasLock === false ? "NO LOCK" + (o.note ? " \u2014 " + o.note : "") : "\u2014"); }); } }
    if ((c.auction || []).length) { h2("Auction units"); c.auction.forEach(function (u) { var a = (d.auction || {})[u.id] || {}; line("Unit " + u.unit, a.note || done(a.done)); }); }
    var g = d.grounds || {};
    h2("Grounds & site");
    line("Grounds condition", g.condition);
    line("Notes", g.note);
    if (d.climate) { h2("Climate control"); line("Temperature", d.climate.temp); line("Notes", d.climate.note); }
    if (rep.note) { h2("Office note on this report"); note(rep.note); }
    h2("Sign-off");
    line("Technician", rep.workerName);
    line("Reviewed", rep.reviewed ? "Yes \u2014 " + (rep.reviewedBy || "") + " on " + fdt(rep.reviewedAt) : "Not yet reviewed");
    y += 24; need(40);
    doc.setDrawColor(150); doc.line(M, y, M + 220, y); doc.line(W - M - 180, y, W - M, y);
    doc.setFontSize(9); doc.setTextColor(110);
    doc.text("Technician signature", M, y + 12); doc.text("Date", W - M - 180, y + 12);

    var total = doc.internal.getNumberOfPages();
    for (var i = 1; i <= total; i++) { doc.setPage(i); doc.setFontSize(8.5); doc.setTextColor(140); doc.text(f.name + " \u2014 " + (rep.type === "audit" ? "Monthly Audit" : "Site Check") + "   Page " + i + " of " + total, M, doc.internal.pageSize.getHeight() - 24); }
    var safe = (f.name || "facility").replace(/[^A-Za-z0-9]+/g, "_");
    doc.save(safe + "_" + (rep.type === "audit" ? "audit" : "sitecheck") + "_" + new Date(rep.submittedAt || Date.now()).toISOString().slice(0, 10) + ".pdf");
  }
  ...
  function collectionsView() {
    ...
    if (sub === "setup") {
      ...
      var card = function (f, isDone) {
        ...
        var headBlock = '<div style="font-weight:800;font-size:18px">' + esc(f.name) + "</div>"
          + '<div style="margin-top:8px"><div style="font-size:16px;font-weight:700;color:#E0913C">Typical Collection Date</div><div style="font-size:18px;font-weight:700">' + esc(c.collectionDay || "Not set") + "</div></div>"
          + '<div style="margin-top:8px"><div style="font-size:16px;font-weight:700;color:#E0913C">Typical employee who handles collections</div><div style="font-size:18px;font-weight:700">' + esc(c.collectionEmployee || "Unassigned") + "</div></div>";
