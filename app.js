...
(function () {
  ...
  function addExpenseForm() {
    var d = S.expDraft || { entries: [] }; if (!d.entries || !d.entries.length) d.entries = [{ id: uid(), facilityId: "", amount: "", source: "American Express", sourceOther: "", description: "", datePaid: "" }];
  ...
  function invoicesView() {
    var picker = invFacPicker(), v = S.invFac === undefined ? "__none" : S.invFac;
    if (v === "__none") return picker + '<div class="empty">Pick a facility to view its invoices.</div>';
    var open = (S.invoices || []).filter(function (i) { return i.status === "open" && i.facilityId === v; });
    return picker + (open.length ? '<div class="stack">' + open.map(function (i) { return invCard(i, false); }).join("") + "</div>" : '<div class="empty">No open invoices for this facility.</div>');
  }
  function paidInvoices() {
    var picker = invFacPicker(), v = S.invFac === undefined ? "__none" : S.invFac;
    if (v === "__none") return picker + '<div class="empty">Pick a facility to view its paid invoices.</div>';
    var paid = (S.invoices || []).filter(function (i) { return i.status === "paid" && i.facilityId === v; });
    return picker + (paid.length ? '<div class="stack">' + paid.map(function (i) { return invCard(i, true); }).join("") + "</div>" : '<div class="empty">No paid invoices for this facility.</div>');
  }
