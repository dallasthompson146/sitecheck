...
app.post("/api/leads", auth, staffOrAbove, (req, res) => {
  ...
  const lead = {
    ...
    notes: b.notes || "", createdAt: Date.now(), createdBy: req.user.name, followups: [],
  };
...
app.put("/api/leads/:id", auth, staffOrAbove, (req, res) => {
  ...
  ["name", "estMoveIn", "facilityId", "phone", "phone2", "email", "notes"].forEach((k) => { if (typeof req.body[k] === "string") l[k] = req.body[k]; });
...
app.get("/api/payables", auth, adminOrAbove, (req, res) => res.json(db.payables.slice().sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))));
app.post("/api/payables", auth, adminOrAbove, (req, res) => {
  ...
  const p = { id: uid(), createdAt: Date.now(), createdBy: req.user.name,
    ...
    cycleDate: b.cycleDate || "", mailingAddress: b.mailingAddress || "", invoice: b.invoice || null,
    status: "pending", billedBack: false };
  db.payables.push(p); persist(); res.json(p);
});
app.put("/api/payables/:id", auth, adminOrAbove, (req, res) => {
  const p = db.payables.find((x) => x.id === req.params.id);
  if (!p) return res.status(404).json({ error: "Not found." });
  ["facilityId", "who", "contractorName", "description", "amount", "cycleDate", "mailingAddress"].forEach((k) => { if (typeof req.body[k] === "string") p[k] = req.body[k]; });
  ...
  if (typeof req.body.billedBack === "boolean") p.billedBack = req.body.billedBack;
  persist(); res.json(p);
});
app.delete("/api/payables/:id", auth, adminOrAbove, (req, res) => { db.payables = db.payables.filter((x) => x.id !== req.params.id); persist(); res.json({ ok: true }); });

app.get("/api/recurring", auth, adminOrAbove, (req, res) => res.json(db.recurring.slice()));
app.post("/api/recurring", auth, adminOrAbove, (req, res) => {
  const b = req.body || {};
...
app.put("/api/recurring/:id", auth, adminOrAbove, (req, res) => {
...
app.delete("/api/recurring/:id", auth, adminOrAbove, (req, res) => { db.recurring = db.recurring.filter((x) => x.id !== req.params.id); persist(); res.json({ ok: true }); });
