...
function blobifyDeep(node) {
  ...
  if (isDataUri(node.url)) { const b = saveBlob(node.url); if (b) { node.blob = b; delete node.url; n++; } }
...
if (fs.existsSync(DATA_FILE)) {
  ...
  db.facilities.forEach((f) => {
    (f.config.facilityPhotos || []).forEach((p) => { if (p && isDataUri(p.url)) { const b = saveBlob(p.url); if (b) { p.blob = b; delete p.url; migrated++; } } });
  });
  (db.receipts || []).forEach((r) => { if (r && isDataUri(r.file)) { const b = saveBlob(r.file); if (b) { r.blob = b; delete r.file; migrated++; } } });
  (db.payables || []).forEach((p) => { if (p && isDataUri(p.invoice)) { const b = saveBlob(p.invoice); if (b) { p.invoiceBlob = b; delete p.invoice; migrated++; } } });
  ...
  if (migrated) { persist(); console.log("Migrated " + migrated + " embedded image(s) out of data.json into blob files."); }
} else {
  db = seed(); persist();
