  ...
  return {
    ...
    facilities: [
      { id: "fac-north", name: "Northgate Self Storage", address: "4120 Industrial Pkwy",
        config: { updatedAt: Date.now() - 3 * D, climateControlled: true,
          ...
          vacated: [{ id: uid(), unit: "B118" }], vacant: [{ id: uid(), unit: "B112" }, { id: uid(), unit: "E501" }], auction: [{ id: uid(), unit: "F602" }] } },
      { id: "fac-south", name: "Southside Storage Depot", address: "88 Commerce Loop",
        config: { updatedAt: Date.now() - D,
          ...
          maintenance: [], vacated: [{ id: uid(), unit: "A210" }], vacant: [{ id: uid(), unit: "A210" }], auction: [] } },
    ],
    submissions: [],
    drafts: [],
    reports: [],
...
app.post("/api/facilities", auth, adminOrAbove, (req, res) => {
  const f = { id: uid(), name: req.body.name || "New facility", address: req.body.address || "",
    config: { updatedAt: Date.now(), weeklyTasks: [], lockoutAdd: [], lockoutRemove: [], lockoutKeep: [], maintenance: [], vacated: [], vacant: [], auction: [] } };
...
const FU_LABEL = { initial4h: "First follow-up (4 hours after lead)", after1day: "Follow-up (1 day later)", preMoveIn: "Day before move-in", moveInDay: "Move-in day", postMoveIn: "Day after move-in", twoWeek: "Two weeks after move-in" };
...
function applyFollowup(lead, fu, form) {
  ...
  } else if (fu.step === "initial4h" && lead.status === "warm") {
...
app.put("/api/leads/:id", auth, staffOrAbove, (req, res) => {
  ...
  if (["warm", "rented", "cold"].includes(req.body.status)) l.status = req.body.status;
