...
app.put("/api/payables/:id", auth, superAdmin, (req, res) => {
  ...
  if (["pending", "approved", "rejected"].includes(req.body.status)) { p.status = req.body.status; p.decidedAt = Date.now(); p.decidedBy = req.user.name;
    if (p.status === "approved" && p.invoice && !p.receiptId) { const rc = { id: uid(), createdAt: Date.now(), name: p.invoiceName || ("Invoice — " + (p.contractorName || "payable")), file: p.invoice, facilityId: p.facilityId || "", source: "", fromPayableId: p.id }; db.receipts.push(rc); p.receiptId = rc.id; }
  }
...
app.post("/api/receipts", auth, superAdmin, (req, res) => {
  const b = req.body || {};
  const rc = { id: uid(), createdAt: Date.now(), createdBy: req.user.name, name: b.name || "Receipt", file: b.file || null, facilityId: b.facilityId || "", source: b.source || "" };
  db.receipts.push(rc); persist(); res.json(rc);
});
app.delete("/api/receipts/:id", auth, superAdmin, (req, res) => { db.receipts = db.receipts.filter((x) => x.id !== req.params.id); persist(); res.json({ ok: true }); });
