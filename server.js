...
app.put("/api/facilities/:id", auth, canEditFacility, (req, res) => {
  ...
  if (typeof req.body.setupNotes === "string") f.setupNotes = req.body.setupNotes;
  if (req.body.config) {
...
/* ---- weekly collections ----
   Schedule lives on each facility (config.collectionDay / collectionEmployee).
   Each completed week is a record in db.collections keyed by facility + week. */
function weekKey(ts) {
...
app.get("/api/collections", auth, staffOrAbove, (req, res) => {
  res.json({ week: weekKey(), collections: (db.collections || []).slice().sort((a, b) => (b.completedAt || 0) - (a.completedAt || 0)) });
});
app.post("/api/collections", auth, staffOrAbove, (req, res) => {
  const b = req.body || {};
  if (!b.facilityId) return res.status(400).json({ error: "Pick a facility." });
  const f = db.facilities.find((x) => x.id === b.facilityId);
  if (!f) return res.status(404).json({ error: "Facility not found." });
  const wk = b.week || weekKey();
  const existing = (db.collections || []).find((x) => x.facilityId === b.facilityId && x.week === wk);
  if (existing) return res.status(409).json({ error: "Collections for this facility are already marked done this week." });
  const rec = { id: uid(), facilityId: b.facilityId, week: wk, completedAt: Date.now(), completedBy: req.user.name, calledAll: !!b.calledAll };
  db.collections.push(rec); persist(); res.json(rec);
});
app.delete("/api/collections/:id", auth, adminOrAbove, (req, res) => {
  db.collections = (db.collections || []).filter((x) => x.id !== req.params.id); persist(); res.json({ ok: true });
});
