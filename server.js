...
function sanitizeFacility(f, role) {
  ...
  if (role === "worker") { delete cfg.maintenanceTracking; delete cfg.inventory; delete cfg.contractors; delete cfg.adminNotes; }
  return { ...f, config: cfg };
}
...
app.put("/api/facilities/:id", auth, adminOrAbove, (req, res) => {
...
app.post("/api/users", auth, adminOrAbove, (req, res) => {
  ...
  let newRole = role === "admin" ? "admin" : role === "superadmin" ? "superadmin" : "worker";
...
app.put("/api/users/:id", auth, adminOrAbove, (req, res) => {
  ...
  if (typeof role === "string" && role && role !== u.role) {
    ...
    u.role = role === "admin" ? "admin" : role === "superadmin" ? "superadmin" : "worker";
