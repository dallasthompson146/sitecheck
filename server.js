...
function preMigrateBlobs(file) {
  if (!fs.existsSync(file)) return false;
  const { StringDecoder } = require("string_decoder");
  const dec = new StringDecoder("utf8");
  const CHUNK = 1 << 20;
  const buf = Buffer.allocUnsafe(CHUNK);
  const outPath = file + ".migrating";
  ...
  if (changed) { fs.renameSync(outPath, file); return true; }
...
if (fs.existsSync(DATA_FILE)) {
  const preChanged = preMigrateBlobs(DATA_FILE); // streams out big embedded images first, so the parse below fits in memory
  if (preChanged) console.log("Pre-migration: streamed embedded images out of data.json into blob files.");
  ...
  // One-time migration: pull any base64 images out of data.json into blob files so memory stays small.
  let migrated = 0;
  db.facilities.forEach((f) => {
    (f.config.facilityPhotos || []).forEach((p) => { if (p) { const b = toBlobName(p.url); if (b) { p.blob = b; delete p.url; migrated++; } } });
  });
  ...
  if (migrated) { persist(); console.log("Migrated " + migrated + " embedded image reference(s) in data.json into blob files."); }
} else {
  db = seed(); persist();
