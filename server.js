...
try { if (fs.existsSync(MIG)) { fs.unlinkSync(MIG); console.log("Cleared a stale data.json.migrating file."); } } catch (e) {}
...
if (!fs.existsSync(DATA_FILE)) {
  ...
  console.log("Startup disk state — data.json: " + fsize(DATA_FILE) + " bytes; .bak: " + fsize(DATA_FILE + ".bak") + " bytes; free-migrate space check on /data blobs: " + (fs.existsSync(BLOB_DIR) ? "ok" : "missing"));
  ...
  if (!loaded) {
    ...
    // If we loaded straight from data.json fine, the temporary .bak is no longer needed.
    if (source === "data.json") { try { if (fs.existsSync(DATA_FILE + ".bak")) fs.unlinkSync(DATA_FILE + ".bak"); } catch (e) {} }
  }
}
